...
from hanri.outcome_intelligence import run_outcome_intelligence
...
def main() -> None:
    p = argparse.ArgumentParser(description="HANRI R39.4 outcome intelligence")
    ...
    result = run_outcome_intelligence(
    ...
    state_path.write_text(json.dumps(result["state"], ensure_ascii=False, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    ...
    print(json.dumps({
        ...
        "evaluated_recommendations": receipt["metrics"]["evaluated_recommendations"],
        "outcome_coverage_rate": receipt["metrics"]["outcome_coverage_rate"],
