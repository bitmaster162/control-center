...
def _material_policy_r36() -> dict[str, object]:
    ...
    policy["heartbeat_integrity_fast_gate_release"] = "R36"
    return policy


def _load_json_r36(path: Path) -> Any:
    candidate = Path(path)
...
def _atomic_write_json_r36(path: Path, value: Any) -> None:
    candidate = Path(path)
    if candidate.name == archive_sqlite.CACHE_JSON_NAME:
...
def copy_latest_outputs_r36(source_root: Path, target_root: Path) -> dict[str, Any]:
...
def install_r36_guard() -> None:
    ...
    r32._material_policy = _material_policy_r36

    integrity.install_r32_integrity_guard()
    core.VERSION = PROGRAM_VERSION
    r30._atomic_copy = r33._atomic_copy_r33

    core.load_json = _load_json_r36
    core.atomic_write_json = _atomic_write_json_r36
    ...
    core.copy_latest_outputs = copy_latest_outputs_r36
