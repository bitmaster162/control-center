param(
    [string]$TaskName = "ControlCenter-HANRI-R30"
)
...
if ($Checks.config_exists) {
    ...
    $Checks.config_program_version = ($ConfigObject.program_version -eq "30.0.0")
    ...
    $Checks.config_state_isolated = ($ConfigObject.state_root -match "ControlCenterHANRIR30")
    $Checks.config_drive_output_isolated = ($ConfigObject.human_output_root -match "HANRI_R30")
}

if ($Checks.run_receipt_exists) {
    $Run = Get-Content $RunReceiptPath -Raw | ConvertFrom-Json
    $Checks.receipt_program_version = ($Run.program_version -eq "30.0.0")
...
if ($Checks.ai_state_exists) {
    ...
    $Checks.state_program_version = ($State.program_version -eq "30.0.0")
...
if ($Checks.human_digest_exists) {
    ...
    $Checks.digest_identifies_r30 = ($FirstLine.Contains($ExpectedDigestIdentity))
    $Checks.digest_does_not_identify_r29 = (-not $FirstLine.Contains($ForbiddenDigestIdentity))
}

if ($Checks.projection_receipt_exists) {
    $Projection = Get-Content $ProjectionReceiptPath -Raw | ConvertFrom-Json
    $Checks.projection_program_version = ($Projection.program_version -eq "30.0.0")
...
$ReceiptPath = Join-Path $ReceiptRoot "R30_RUNTIME_READBACK_RECEIPT.json"
