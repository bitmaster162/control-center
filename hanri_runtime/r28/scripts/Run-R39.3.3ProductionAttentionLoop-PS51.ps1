...
$runId = 'R39.3.3-PRODUCTION-' + (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
...
# Preserve R39.3.1 self-feedback containment while removing regression tests from
# the recurring production path. CI/install preflight remains responsible for tests.
if (Test-Path -LiteralPath $scannedReceiptRoot -PathType Container) {
  ...
  foreach ($candidate in @($workRoot, $stateRoot, $loopReceiptRoot)) {
    ...
    if ($resolved.StartsWith($scanRoot + '\', [System.StringComparison]::OrdinalIgnoreCase)) {
      throw 'v2_continuous_root_inside_scanned_receipt_root'
    }
  }
}
...
if ($LASTEXITCODE -ne 0) { throw 'R39.3.3 producer adapter run failed' }
...
if ($LASTEXITCODE -ne 0) { throw 'R39.3.3 semantic attention fabric run failed' }
...
if ($LASTEXITCODE -ne 0) { throw 'R39.3.3 semantic continuous loop failed' }
...
Write-Host 'HANRI_R39_3_3_PRODUCTION_LOOP_PASS'
