...
$packageRoot = Join-Path $env:LOCALAPPDATA 'ControlCenterHANRIR39\r39_6_1_runtime_refresh'
$receiptRoot = Join-Path $packageRoot 'receipts'
$backupRoot = Join-Path $packageRoot 'backups'
$planPath = Join-Path $packageRoot 'R39_6_1_RUNTIME_REFRESH_PLAN.json'
$applyReceiptPath = Join-Path $receiptRoot 'R39_6_1_RUNTIME_REFRESH_RECEIPT.json'
...
function Get-SourceManifest {
  ...
  foreach ($root in @($runtime, $decisionReceipt)) {
    ...
    else { $sourceFiles = @(Get-Item -LiteralPath $root) }
...
function Stage-Runtime([string]$Destination) {
  ...
  $r3951CorePath = Join-Path $scripts 'Invoke-R39.5.1AttentionHeartbeat-Core-PS51.ps1'
  $r3961WrapperPath = Join-Path $scripts 'Invoke-R39.6.1AttentionHeartbeat-Wrapper-PS51.ps1'
  $r3951LearningRunner = Join-Path $scripts 'Run-R39.5.1ImprovementLearningLive-PS51.ps1'
  $r3961RecommendationRunner = Join-Path $scripts 'Run-R39.6.1BoundedRecommendationsLive-PS51.ps1'

  foreach ($required in @($taskPath,$r394WrapperPath,$r3951WrapperPath,$r3961WrapperPath,$r3951LearningRunner,$r3961RecommendationRunner)) {
    if (-not (Test-Path -LiteralPath $required -PathType Leaf)) { throw "staged runtime required file missing: $required" }
  }
  foreach ($collision in @($r393CorePath,$r394CorePath,$r3951CorePath)) {
    if (Test-Path -LiteralPath $collision) { throw "staged transform collision: $collision" }
  }
  ...
  Move-Item -Force -LiteralPath $taskPath -Destination $r3951CorePath
  Copy-Item -Force -LiteralPath $r3961WrapperPath -Destination $taskPath
}
...
$planCore = [ordered]@{
  ...
  operation = 'refresh_staged_recommendation_runtime_without_scheduler_xml_change'
  task_name = $TaskName
  source_head = $head
  source_tree = $tree
  ...
  upstream_integration = 'R39.5.1'
  recommendation_integration = 'R39.6.1'
...
$approvalExpected = "APPROVE_R39_6_1_RUNTIME_REFRESH:$actionHash"
...
if (-not $Apply) {
  Write-Host 'HANRI_R39_6_1_RUNTIME_REFRESH_PLAN_PASS'
  Write-Host "HEAD $head"
  Write-Host "TREE $tree"
  Write-Host "SOURCE_MANIFEST_SHA256 $($sourceManifest.manifest_sha256)"
  Write-Host "SOURCE_FILE_COUNT $($sourceManifest.file_count)"
  ...
  Write-Host "BEFORE_RUNTIME_FILE_COUNT $($beforeInstalledManifest.file_count)"
...
if ($ApprovalCommand -ne $approvalExpected) { throw 'exact R39.6.1 runtime refresh approval command mismatch' }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw "R39.6.1 staged heartbeat preflight failed exit=$LASTEXITCODE" }

  $preflightIntegration = Join-Path $preflightRoot 'heartbeat_receipts\R39_6_1_RECOMMENDATION_INTEGRATION_RECEIPT.json'
  if (-not (Test-Path -LiteralPath $preflightIntegration -PathType Leaf)) { throw 'R39.6.1 preflight integration receipt missing' }
  $preflight = Get-Content -Raw -Encoding UTF8 $preflightIntegration | ConvertFrom-Json
  if ($preflight.policy_version -ne '39.6.1-autonomous-recommendation-integration-v1') { throw 'R39.6.1 preflight policy mismatch' }
  if ($preflight.status -ne 'PASS') { throw "R39.6.1 preflight status=$($preflight.status)" }
  if ([bool]$preflight.recommendation_pending) { throw 'R39.6.1 preflight recommendation_pending=true' }
  if ([int]$preflight.execution_effects_performed -ne 0) { throw 'R39.6.1 preflight effects nonzero' }
  ...
  try {
    ...
    $leaseBytes = [System.Text.Encoding]::UTF8.GetBytes("runtime_refresh=r39.6.1`npid=$PID`nutc=$([DateTime]::UtcNow.ToString('o'))`n")
    $leaseHandle.Write($leaseBytes,0,$leaseBytes.Length)
    $leaseHandle.Flush()
    $leaseOwned = $true
  }
  catch [System.IO.IOException] { throw 'live_attention_lease_busy' }
  ...
  [ordered]@{
    ...
    target_runtime_file_count = $targetManifest.file_count
    prior_runtime_backup = $backup
    preflight_root = $preflightRoot
    preflight_recommendation_integration_passed = $true
  ...
  Write-Host 'HANRI_R39_6_1_RUNTIME_REFRESH_APPLY_PASS'
  Write-Host "HEAD $head"
  Write-Host "TREE $tree"
  Write-Host "ACTION_HASH $actionHash"
  Write-Host "TASK_XML_SHA256 $($afterTask.xml_sha256)"
  Write-Host "RUNTIME_MANIFEST_SHA256 $($afterInstalled.manifest_sha256)"
  Write-Host "RUNTIME_FILE_COUNT $($afterInstalled.file_count)"
  Write-Host 'PREFLIGHT_RECOMMENDATION_INTEGRATION_PASSED true'
  ...
  try {
    if ($swapStarted) {
      if (Test-Path -LiteralPath $InstallRoot -PathType Container) { Remove-Item -Recurse -Force -LiteralPath $InstallRoot }
      if (-not (Test-Path -LiteralPath $backup -PathType Container)) { throw 'runtime backup missing during rollback' }
