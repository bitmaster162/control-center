...
$config = Join-Path $runtime 'config\r39.2.1.attention-coverage-closure.json'
...
$bundle = Join-Path $producerDir 'R39_2_1_PRODUCER_ENVELOPES.json'
$adapterReceipt = Join-Path $receiptDir 'R39_2_1_COVERAGE_CLOSURE_RECEIPT.json'
$fabricReceipt = Join-Path $receiptDir 'R39_2_1_ATTENTION_FABRIC_RECEIPT.json'
$generatedAt = (Get-Date).ToUniversalTime().ToString('yyyy-MM-ddTHH:mm:ssZ')
$runId = 'R39.2.1-HOST-' + (Get-Date).ToUniversalTime().ToString('yyyyMMddTHHmmssZ')
...
if ($LASTEXITCODE -ne 0) { throw "R39.2.1 regression tests failed" }

python -m hanri.producer_adapters_coverage_cli `
  --config $config `
  --output-bundle $bundle `
  --output-receipt $adapterReceipt `
  --generated-at $generatedAt
if ($LASTEXITCODE -ne 0) { throw "R39.2.1 producer adapter run failed" }

# Isolate this run from prior R39.2 bundle files so the fabric sees only current R39.2.1 evidence.
$fabricInput = Join-Path $OutputRoot 'producer_current_r39_2_1'
...
Copy-Item -LiteralPath $bundle -Destination (Join-Path $fabricInput 'R39_2_1_PRODUCER_ENVELOPES.json') -Force
...
if ($LASTEXITCODE -ne 0) { throw "R39.2.1 attention fabric run failed" }
...
if ($a.policy_version -ne '39.2.1-attention-coverage-closure-v1') { throw 'policy_version_mismatch' }
...
Write-Host 'HANRI_R39_2_1_ATTENTION_COVERAGE_CLOSURE_PASS'
