param(
    ...
    [string]$TaskName = "ControlCenter-HANRI-R28"
)

$ErrorActionPreference = "Stop"
$SourceRoot = (Resolve-Path "$PSScriptRoot\..").Path
$InstallRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR28\app"
$StateRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR28\state"
$LogRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR28\logs"
$MigrationRoot = Join-Path $StateRoot "migration"
$PriorTasks = @("ControlCenter-HANRI-R24", "ControlCenter-HANRI-R25", "ControlCenter-HANRI-R26", "ControlCenter-HANRI-R27")
...
if (-not $Apply) {
    ...
    Write-Host "Prior HANRI tasks will be exported and disabled after a successful R28 initial run unless -KeepPriorTasks is used."
...
$Config = Join-Path $InstallRoot "config\r28.windows.json"
...
if ($LASTEXITCODE -ne 0) { throw "R28 compile check failed with exit code $LASTEXITCODE" }
& $Python -m hanri once --config $Config
if ($LASTEXITCODE -ne 0) { throw "Initial HANRI R28 run failed with exit code $LASTEXITCODE" }
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger @($TriggerLogon, $TriggerMinute) -Settings $Settings -Description "HANRI R28 truth-kernel causal bounded recursive improvement supervisor" -Force | Out-Null
