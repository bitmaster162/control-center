...
$ExpectedBranch = "hanri/r29-release-candidate-2"
$ExpectedDigestHeader = "# Human Decision Digest — HANRI R29"
...
function Assert-R29RuntimeReadback([string]$RunReceiptPath, [string]$AiStatePath, [string]$DigestPath) {
    ...
    if (-not $Digest.StartsWith($ExpectedDigestHeader)) {
        throw "RC2 identity readback failed: human digest is not HANRI R29"
    }
}
...
Write-Host "HANRI R29 RC2 side-by-side install gate"
...
try {
    ...
    $ReceiptPath = Join-Path $ReceiptRoot "INSTALL_R29_RC2_RECEIPT.json"
    $Receipt | ConvertTo-Json -Depth 10 | Set-Content -Encoding UTF8 $ReceiptPath
    Write-Host "PASS: HANRI R29 RC2 installed side-by-side and scheduled readback verified."
