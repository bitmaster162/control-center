param(
    ...
    [string]$TaskName = "ControlCenter-HANRI-R26"
)

$ErrorActionPreference = "Stop"
$SourceRoot = (Resolve-Path "$PSScriptRoot\..").Path
$InstallRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR26\app"
$StateRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR26\state"
$LogRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR26\logs"
$MigrationRoot = Join-Path $StateRoot "migration"
$PriorTasks = @("ControlCenter-HANRI-R24", "ControlCenter-HANRI-R25")
...
if (-not $Apply) {
    ...
    Write-Host "Prior HANRI tasks will be exported and disabled after a successful R26 initial run unless -KeepPriorTasks is used."
...
$Config = Join-Path $InstallRoot "config\r26.windows.json"
...
if ($LASTEXITCODE -ne 0) { throw "R26 compile check failed with exit code $LASTEXITCODE" }
& $Python -m hanri once --config $Config
if ($LASTEXITCODE -ne 0) { throw "Initial HANRI R26 run failed with exit code $LASTEXITCODE" }
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger @($TriggerLogon, $TriggerMinute) -Settings $Settings -Description "HANRI R26 causal-spine bounded recursive improvement supervisor" -Force | Out-Null
