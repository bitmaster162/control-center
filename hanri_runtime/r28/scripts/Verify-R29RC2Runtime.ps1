...
if ($Checks.human_digest_exists) {
    ...
    $Checks.digest_identifies_r29 = $Digest.StartsWith($ExpectedDigestHeader)
    $Checks.digest_does_not_identify_r28 = (-not $Digest.StartsWith("# Human Decision Digest — HANRI R28"))
}
...
$ReceiptPath = Join-Path $ReceiptRoot "R29_RC2_RUNTIME_READBACK_RECEIPT.json"
