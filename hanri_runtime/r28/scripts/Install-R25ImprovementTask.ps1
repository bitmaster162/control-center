...
$StateRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR25\state"
$LogRoot = Join-Path $env:LOCALAPPDATA "ControlCenterHANRIR25\logs"
...
$Config = Join-Path $InstallRoot "config\r25.windows.json"
...
Register-ScheduledTask -TaskName $TaskName -Action $Action -Trigger @($TriggerLogon, $TriggerMinute) -Settings $Settings -Description "HANRI R25 bounded recursive improvement supervisor" -Force | Out-Null
