...
def registry_candidate(kind: str, *, challenge_id: str = CHALLENGE, nonce_sha: str = NONCE):
    body = {
        "schema": kind,
...
def approval_fixture():
    next_nonce = registry_candidate("control_center.shadow_human_nonce_epoch_registry_snapshot.v1")
    next_cred = registry_candidate("control_center.shadow_human_credential_registry_snapshot.v1")
    body = {
...
class HumanGateAtomicConsumeTests(unittest.TestCase):
    def build_chain(self):
        ...
        compare = build_human_gate_consume_compare(
            prepare, state, expected_current_state_sha256=state["state_sha256"]
        )
    ...
    def test_compare_rejects_state_change_after_prepare(self):
        ...
        changed = build_human_gate_state_snapshot(
            state_id=state["state_id"], authority_root_sha256=ROOT, generation=11,
            credential_registry_sha256=PRIOR_CRED, nonce_registry_sha256=PRIOR_NONCE,
        ...
        with self.assertRaisesRegex(HumanGateAtomicConsumeError, "compare_and_swap_state_changed"):
            build_human_gate_consume_compare(
                prepare, changed, expected_current_state_sha256=changed["state_sha256"]
            )

    def test_two_prepares_from_same_state_do_not_survive_first_hypothetical_commit(self):
        approval = approval_fixture()
        state = state_fixture()
    ...
    def test_durable_commit_overclaim_is_rejected_even_if_rehashed(self):
        ...
        forged["commit_candidate_sha256"] = sha256_obj(
            {k: v for k, v in forged.items() if k != "commit_candidate_sha256"}
        )
        with self.assertRaisesRegex(HumanGateAtomicConsumeError, "atomic_durable_commit_overclaim"):
            build_human_gate_atomic_consume_verification(
                prepare, compare, forged,
